 ...
 */
class BalanceSheet extends DolibarrModules
{
	/**
    ...
    public function get_balancesheet($databasetable){	
        ...
        SUM(tax) AS Taxes,
        SUM(interest_payments) AS Interest_Payments,
        SUM(invoices) - ((SUM(purchase_orders) + SUM(expenses) + SUM(tax) + SUM(interest_payments))) AS shareholder_equity
    FROM
        (SELECT 
        month_id,
        mname,
    ...
    FROM
        ...
        foreach($b as $c=>$d){
            if(!is_integer($c)){
                $t .= '<tr><td><b>'.$c.'</b></td><td style="text-align:right">'.$d.'</td></tr>'; 
            }                     
        }
        $t .= '</table><br>';
