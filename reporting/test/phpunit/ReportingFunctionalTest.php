...
/**
 * \file    test/phpunit/ReportingFunctionalTest.php
 * \ingroup reporting
 ...
 */
class ReportingFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modReporting')->click();
		} else {
			// Disable the module
			$this->byHref('modReporting')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/reporting/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('reporting/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/reporting/admin/about.php');
		$this->authenticate();
		return $this->assertContains('reporting/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('reportingwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modReporting_ReportingTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modReporting_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
